function h = highlowFlex(guiHandle, hi,lo,cl,op,color,dates,dateform)  
...
if nargin < 3  
  error(message('finance:highlow:missingInputs'))  
end  
  
[m,n] = size(hi);  
if size(hi, 2) > 1 | size(lo, 2) > 1 | size(cl, 2) > 1 | (exist('op', 'var') & size(op, 2) > 1), 
  error(message('finance:highlow:invalidInputSize')) 
elseif size(hi, 1) ~= size(lo, 1) | size(lo, 1) ~= size(cl, 1) | size(cl, 1) ~= size(hi, 1),
	error(message('finance:highlow:mismatchInputDataSize'));
end 
if nargin == 6 | nargin == 7,
	if size(dates, 2) ~= 1,
		error(message('finance:highlow:invalidDatesSize'));
	elseif size(dates, 1) ~= size(hi, 1),
		error(message('finance:highlow:mismatchDatesData'));
	end
end
