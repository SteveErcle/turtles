...
for i = 0:futer/interval
    ...
    for isq = 10:10:100
        ...
        sMod = SignalGenerator(stock, present+2-isq, sampLen);
        [sigMod, sigHL, sigH, sigL] = sMod.getSignal ('c', filtH, filtL);
        
        present+2-isq;
        
        sPro = SignalGenerator(stock, present+2+predLen-isq, sampLen+predLen);
        [sigPro, sigHL, sigH, sigL] = sPro.getSignal('c', filtH, filtL);
        
        present+2+predLen-isq;
        ...
        %         title('SQ');
        e = Evaluator(sigMod, model, prediction);
        prPast = [prPast; e.percentReturn(sigPro)];
    end
